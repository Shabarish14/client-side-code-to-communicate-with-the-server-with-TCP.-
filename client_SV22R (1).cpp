...
void client_init(void)
{
    ...
    printf("Connected to the server on socket %d\n",serversockfd); 
     ...
     // New Code lines inserted
     packet_Output = (char*) malloc(MAXBUFLEN);
...
void process_server_message(Packet *packet)
{
    ...
    char* Pack_Buffer = (char*) malloc(MAXIPLEN);
    memcpy(Pack_Buffer, packet->data, MAXIPLEN);
    strcat(packet_Output, Pack_Buffer);
    if(Final_time != 0) {
        interval += current_time - Final_time;
        Message_Count++;
    }
